...
def index(request:Request):
 c=db();tracks=c.execute('SELECT * FROM tracks ORDER BY created_at DESC LIMIT 100').fetchall();playlists=c.execute('SELECT * FROM playlists ORDER BY name').fetchall();connected=c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='spotify_tokens'").fetchone() and c.execute('SELECT 1 FROM spotify_tokens WHERE id=1').fetchone() is not None;c.close();return templates.TemplateResponse('index.html',{'request':request,'tracks':tracks,'playlists':playlists,'spotify_connected':connected})
@app.get('/health')
...
def retry(spotify_id:str):
 c=db();c.execute("UPDATE tracks SET status='queued',progress=0,error=NULL,updated_at=CURRENT_TIMESTAMP WHERE spotify_id=? AND status='failed'",(spotify_id,));c.commit();c.close();return {'ok':True}
...
async def metube_add(url:str=Form(...)):
 ...
 async with httpx.AsyncClient(timeout=20) as client:
  try:r=await client.post(f'{METUBE_URL}/add',json=payload);r.raise_for_status()
  except httpx.HTTPError as e:return {'ok':False,'error':str(e)}
