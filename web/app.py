...
def index(request:Request):
    c=db(); tracks=c.execute('SELECT * FROM tracks ORDER BY created_at DESC LIMIT 100').fetchall(); playlists=c.execute('SELECT * FROM playlists ORDER BY name').fetchall(); connected=c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='spotify_tokens'").fetchone() and c.execute('SELECT 1 FROM spotify_tokens WHERE id=1').fetchone() is not None; c.close()
