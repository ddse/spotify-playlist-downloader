...
def db():
    ...
    c.execute('CREATE TABLE IF NOT EXISTS oauth_state(state TEXT PRIMARY KEY, verifier TEXT NOT NULL, created_at INTEGER NOT NULL)')
    c.commit(); return c
...
def authorize_url():
    verifier,challenge=pkce(); state=secrets.token_urlsafe(32)
    c=db(); c.execute('DELETE FROM oauth_state WHERE created_at<?',(int(time.time())-900,)); c.execute('INSERT INTO oauth_state VALUES(?,?,?)',(state,verifier,int(time.time()))); c.commit(); c.close()
    q={'response_type':'code','client_id':CLIENT_ID,'scope':SCOPES,'redirect_uri':REDIRECT_URI,'state':state,'code_challenge_method':'S256','code_challenge':challenge}
    return 'https://accounts.spotify.com/authorize?'+urlencode(q)
...
async def exchange(code,state):
    ...
    data={'grant_type':'authorization_code','code':code,'redirect_uri':REDIRECT_URI,'client_id':CLIENT_ID,'code_verifier':row['verifier']}
    headers={}
    if CLIENT_SECRET:
        raw=base64.b64encode(f'{CLIENT_ID}:{CLIENT_SECRET}'.encode()).decode(); headers['Authorization']='Basic '+raw; data.pop('client_id',None)
    async with httpx.AsyncClient(timeout=20) as x:
        r=await x.post('https://accounts.spotify.com/api/token',data=data,headers=headers); r.raise_for_status(); token=r.json()
...
async def access_token():
    ...
    if CLIENT_SECRET:
        headers['Authorization']='Basic '+base64.b64encode(f'{CLIENT_ID}:{CLIENT_SECRET}'.encode()).decode(); data.pop('client_id',None)
    async with httpx.AsyncClient(timeout=20) as x:
...
async def public_search(q):
    token=await access_token()
    if token: return await api('/search',{'q':q,'type':'track','limit':20})
    ...
    async with httpx.AsyncClient(timeout=20) as x:
        ...
        r=await x.get('https://api.spotify.com/v1/search',params={'q':q,'type':'track','limit':20},headers={'Authorization':'Bearer '+at}); r.raise_for_status(); return r.json()

async def playlist_items(pid):
    out=[]; offset=0
    while True:
        data=await api(f'/playlists/{pid}/items',{'limit':100,'offset':offset,'fields':'items(item(id,name,artists,album,external_urls,duration_ms,is_local,type)),next,total'})
