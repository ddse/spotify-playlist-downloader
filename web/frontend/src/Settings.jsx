  ...
  return <div className="glass mt-4 rounded-2xl p-5">
    ...
    <div className="mt-5 rounded-xl border border-white/10 bg-black/20 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2"><span className="rounded-lg border border-white/10 px-3 py-1.5 text-xs">{wireguard?.status||'loading'}</span><button onClick={()=>toggleWireguard(!wireguard?.enabled)} disabled={togglingWireguard||wireguard===null||!wireguard?.config_exists} className={'rounded-lg px-3 py-1.5 text-xs text-white '+(wireguard?.enabled?'bg-red-600 hover:bg-red-500':'bg-emerald-600 hover:bg-emerald-500')}>{togglingWireguard?'Applying...':wireguard?.enabled?'Disable':'Enable'}</button></div>
      </div>
