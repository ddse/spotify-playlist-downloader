...
export default function Settings({onClose}) {const {t}=useLanguage();
  ...
  useEffect(()=>{
    ...
    const connect=()=>{
      if(stopped)return;
      ws=new WebSocket(proto+'://'+location.host+'/ws/wireguard');
      ws.onmessage=e=>{try{
  ...
  const toggleWireguard=async(enabled)=>{
    ...
    setMessage('');
    setWireguard(x=>({...x,requested_enabled:enabled,operation:enabled?'connecting':'disconnecting',status:enabled?'connecting':'disconnecting'}));
    try{
      ...
      setMessage(enabled?t('WireGuard connecting...'):t('WireGuard disconnecting...'));
    }catch(e){
  ...
  const toggleSchedule=async(enabled)=>{
    setSavingSchedule(true); setMessage('');
    try{
      const d=await api('/api/settings/schedule',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled})});
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-3 backdrop-blur-md md:p-6" role="dialog" aria-modal="true" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-zinc-950 shadow-2xl shadow-black/50">
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 md:px-5">
        <div className="flex items-center gap-3"><div className="rounded-lg bg-violet-600/15 p-2 text-violet-300"><Settings2 size={18}/></div><div><h2 className="font-semibold">{t('Settings')}</h2><p className="text-[11px] text-zinc-500">{t('Provider connections and network')}</p></div></div>
        <button type="button" onClick={onClose} className="rounded-lg p-2 text-zinc-400 hover:bg-white/5 hover:text-white" aria-label={t('Close settings')}><X size={18}/></button>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4 md:p-5">
        {selected==='schedule' ? <div className="space-y-4">
          ...
          <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[.02] p-4">
            ...
            <button type="button" role="switch" aria-checked={scheduleEnabled} disabled={savingSchedule} onClick={()=>toggleSchedule(!scheduleEnabled)} className={'relative h-6 w-11 rounded-full transition '+(scheduleEnabled?'bg-violet-600':'bg-zinc-700')}>
              <span className={'absolute top-1 h-4 w-4 rounded-full bg-white transition '+(scheduleEnabled?'left-6':'left-1')}/>
            </button>
          </div>
          {message&&<div className="text-xs text-zinc-400">{message}</div>}
        </div> : selected==='wireguard' ? <div className="space-y-4">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-medium">{t('WireGuard')}</h3><p className="mt-1 text-xs text-zinc-500">{t('Network tunnel configuration and connection status.')}</p></div><div className="flex items-center gap-2"><span className="rounded-full border border-white/10 px-2.5 py-1 text-[11px] text-zinc-300">{wireguard?.status||t('loading')}</span><button type="button" onClick={()=>toggleWireguard(!wireguard?.enabled)} disabled={togglingWireguard||wireguard===null||!!wireguard?.operation||(!wireguard?.enabled&&!wireguard?.configured)} className={'rounded-lg px-3 py-1.5 text-xs font-medium text-white '+(wireguard?.enabled?'bg-red-600 hover:bg-red-500':'bg-emerald-600 hover:bg-emerald-500')}>{wireguard?.operation==='connecting'?t('Connecting...'):wireguard?.operation==='disconnecting'?t('Disconnecting...'):wireguard?.enabled?t('Disable'):t('Enable')}</button></div></div>
          <div className="grid gap-2 sm:grid-cols-3">{[[t('Interface'),wireguard?.interface||'wg0'],[t('Config'),wireguard?.configured?'Saved':'Missing'],[t('Route'),wireguard?.route_active?'Active':'Inactive'],[t('Handshake'),wireguard?.handshake_recent?'Recent':'Not recent'],[t('Public IP'),wireguard?.public_ip||'—'],[t('Peers'),wireguard?.peer_count??0]].map(([label,value])=><div key={label} className="rounded-xl border border-white/10 bg-white/[.02] p-3"><div className="text-[10px] uppercase tracking-wide text-zinc-500">{label}</div><div className="mt-1 text-sm text-zinc-200">{value}</div></div>)}</div>
          <div className="rounded-xl border border-white/10 bg-black/20 p-3"><label><span className="mb-2 block text-xs font-medium text-zinc-400">{t('Client configuration')}</span><textarea value={wireguardConfig} onChange={e=>setWireguardConfig(e.target.value)} placeholder={wireguard?.configured?t('Configuration is saved and hidden. Paste a new config to replace it.'):t('Paste the contents of wg0.conf here')} className="min-h-32 w-full rounded-lg border border-white/10 bg-black/20 p-3 font-mono text-[11px] outline-none focus:border-violet-500"/></label><div className="mt-2 flex items-center gap-2"><button onClick={saveWireguard} disabled={savingWireguard||!wireguardConfig.trim()} className="rounded-lg bg-violet-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-40">{savingWireguard?t('Saving...'):t('Save configuration')}</button>{wireguard?.configured&&<span className="text-[11px] text-emerald-300">{t('Saved (write-only)')}</span>}</div></div>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[.02] p-4">
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-white/5 pb-4"><div><h3 className="font-medium">{p.name}</h3><p className="mt-1 text-xs text-zinc-500">{currentConfigured?t('Configuration saved'):t('No configuration saved yet')}</p></div><label className="flex cursor-pointer items-center gap-2 text-xs text-zinc-300"><input type="checkbox" checked={item.enabled!==false} onChange={e=>setItems(x=>({...x,[selected]:{...item,enabled:e.target.checked}}))}/> Enabled</label></div>
            <div className="mt-4 grid gap-3 md:grid-cols-2">{p.fields.map(([k,label,type])=><label key={k} className={type==='textarea'?'md:col-span-2':''}><span className="mb-1.5 block text-[11px] text-zinc-500">{t(label)}</span>{type==='textarea'?<textarea value={config[k]||''} onChange={e=>set(k,e.target.value)} className="min-h-28 w-full rounded-lg border border-white/10 bg-black/20 p-2.5 text-xs outline-none focus:border-violet-500"/>:<input type={type||'text'} value={config[k]||''} onChange={e=>set(k,e.target.value)} className="w-full rounded-lg border border-white/10 bg-black/20 p-2.5 text-sm outline-none focus:border-violet-500"/>}</label>)}</div>
            {p.fields.length===0&&<div className="mt-4 rounded-lg border border-dashed border-white/10 p-3 text-xs text-zinc-500">{t('No credentials are required for this provider.')}</div>}
            <div className="mt-5 flex flex-wrap items-center gap-2"><button onClick={save} disabled={saving} className="rounded-lg bg-violet-600 px-3.5 py-2 text-xs font-medium text-white disabled:opacity-40">{saving?t('Saving...'):t('Save changes')}</button><button onClick={test} disabled={testing} className="rounded-lg border border-white/10 px-3.5 py-2 text-xs">{testing?t('Testing...'):t('Test connection')}</button>{message&&<span className="text-xs text-zinc-400">{message}</span>}{item.status&&<span className={'ml-auto flex items-center gap-1 text-[11px] '+statusTone}><CheckCircle2 size={13}/> {item.status}</span>}</div>
          </div>
        </div>}
      </div>
    </div>
