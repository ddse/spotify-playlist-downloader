...
def test_retry_completed_track_requeues(tmp_path, monkeypatch):
    ...
    _seed(connect, "completed")
    r = client.post("/api/retry", params={"track_id": TRACK_ID})
    assert r.status_code == 200
    assert r.json() == {"ok": True}
    c = connect()
    row = c.execute("SELECT status,progress,error,download_speed,eta FROM tracks WHERE spotify_id=?", (TRACK_ID,)).fetchone()
    c.close()
...
def test_retry_failed_track_requeues_and_clears_error(tmp_path, monkeypatch):
    connect, client = _client(tmp_path, monkeypatch)
    _seed(connect, "failed", error="download failed")
    r = client.post("/api/retry", params={"track_id": TRACK_ID})
    assert r.status_code == 200
    assert r.json() == {"ok": True}
    c = connect()
    row = c.execute("SELECT status,progress,error FROM tracks WHERE spotify_id=?", (TRACK_ID,)).fetchone()
    c.close()
    assert tuple(row) == ("queued", 0, None)
...
def test_queue_actions_accept_encoded_track_id_as_query_parameter(tmp_path, monkeypatch):
    ...
    c = connect()
    row = c.execute("SELECT status,auto_start,priority FROM tracks WHERE spotify_id=?", (TRACK_ID,)).fetchone()
    c.close()
...
def test_frontend_track_actions_use_query_parameters(tmp_path, monkeypatch):
