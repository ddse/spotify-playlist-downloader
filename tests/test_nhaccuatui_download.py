...
def test_search_keyword_returns_input_song_link(monkeypatch):
    nct = load_provider()
    html = f"""
    <a href="{SONG_LINK}"><span>Hoa Vô Sắc</span></a>
    <a href="{SONG_LINK}"><span>duplicate</span></a>
    """
    ...
    assert result["items"][0]["url"] == SONG_LINK
    assert result["items"][0]["id"] == SONG_LINK
...
def test_download_selects_active_non_vip_320_stream():
    detail = {
        ...
        "streamURL": [
            ...
            },
            {
                ...
                "onlyVIP": True,
                "status": 0,
                "stream": "https://cdn.example.test/song.flac",
            },
        ],
    }
    ...
    streams = [
        item
        for item in detail["streamURL"]
        if item.get("status") == 1
        and not item.get("onlyVIP", False)
        and item.get("stream")
    ]
...
def test_download_writes_file(tmp_path, monkeypatch):
    ...
    with open(output, "wb") as file:
        with fake_urlopen(
            __import__("urllib.request", fromlist=["Request"]).Request(STREAM_320),
            timeout=30,
        ) as response:
